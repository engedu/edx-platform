define(['js/views/validation',
    ...
    function (ValidatingView, $, _, gettext, CodeMirror, ValidationErrorModal, HtmlUtils) {
        var NotifyView = ValidatingView.extend({
            ...
            render: function () {
                ...
                var listEle$ = this.$el.find('.course-notification');
                listEle$.empty();
                ...
                _.each(_.sortBy(_.keys(this.model.attributes.child_info.children), function (key) {                  
            ...
            renderTemplate: function (key, model) {
                var status;
                for(var i = 0; i < model.child_info.children.length; i++){
                    if(model.child_info.children[i].due_date != ""){
                        status = true;
                        break;
                    }
                }
                var newKeyId = _.uniqueId('policy_key_'),
                    newEle = this.template({
                        ...
                        children: model.child_info.children,
